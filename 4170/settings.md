...
3. Настройки **Carrot Server** имеют 8 разделов: `Common`, `Data Base`, `Digital Assets`, `GPIO`, `III Protocol Commands`, `Licence`, `Replication`, `WebSocket`.

4. Раздел `Common`, включает следующие поля для редактирования:
- `Server Name` - отображаемое имя сервера в **System Monitor**.

5. Раздел `DataBase`, включает следующие поля для редактирования:
...
6. Раздел `Digital Assets`, включает следующие поля для редактирования:
...
7. Раздел `GPIO`, включает следующие поле для редактирования:
- `Keep UE Event Active Status` - при `True` нажатие на событие из плейлиста вызывает один и тот же стейт.

8. Раздел `III Protocol Commands`, включает следующее поле для редактирования:
...
9. Раздел `Licence`, включает следующие поля для редактирования:
- `Licence Key` - поля для внесения лицензионного ключа **Carrot**.

10. Раздел `Replication`, включает следующие поля для редактирования:
...
11. Раздел `WebSocket`, включает следующие поля для редактирования:
...
12. Нажмите кнопку `Save` и закройте окно настроек.
...
Кнопка `Delete settings` удаляет выбранный пресет.
