...
Для того, чтобы Carrot распознал отрезки анимации, пользователю требуется выставить маркеры композиции. Композиция должна содержать минимум 3 маркера.
...
Область между маркерами с одинаковыми именами (в примере `ANIM`-`ANIM`) игнорируется в Carrot и пропускается при воспроизведении.

### Рекомендации к подготовке шаблона

На 01.11.2021 для корректной работы шаблонов представлены следующие рекомендации к подготовке проекта:
...
- Между ключем анимации и маркером рекомендуется оставлять расстояние около 3 кадров.

#### Эффекты
Список поддерживаемых эффектов на 01.11.2021:
...
- Для максимальной оптимизации PSD/AI слои внутри композиции рекомендуется заменить на Solid Layer, либо на готовое изображение PNG или JPG.

Поддерживаемые типы слоев в композиции:
- Null Layer
- Solid Layer
- Shape Layer в виде Bezier Path (без эффектов)
- Text Layer
...
7. Для назначения слоя, содержащего текст, в качестве переменной, выполните следующее:
    ...
    - В поле `Name` введите желаемое имя переменной.
    - В поле `Type` укажите тип `Text`. В случае, если указан тип `RichText`, к содержимому поля `DefaultValue` добавится тег изначального цвета текста:
