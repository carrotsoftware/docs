...
>Стоит учитывать, что при включении параметров `Windows Restricted Chars` и `Event Autonaming` (располагается в настройках веб-интерфейса **Web-Playlist (Carrot Dashboard)**) невозможно использовать настройку **"Use for Autonaming"** в шаблоне, содержащем переменные типа **RichText** и **Color**. В случае включения настройки **"Use for Autonaming"** наблюдается некорректная работа формируемых ивентов шаблона.
...
>
>При работе с приложением **Carrot Server** без лицензионного ключа, необходимо убедиться, что у рабочей станции включена поддержка **Интернет протокола IPv6**.
...
| | |
|-------------------|----------|
| `Dummy Filepath`      | Указание директории (пути) к изображению, заменяющему основной выход генерируемого сигнала **Carrot Engine**. Применимо для проведения технической профилактики. |

>Присвоенное изображение параметру `Dummy Filepath` используется при взаимодействии с кнопкой `Dummy Output` приложения **Carrot Engine**.
...
| `Always show Event states`                               | Включение режима, при котором генерируется наименование ивента на основе наименования шаблона и значений его переменных |

>Стоит учитывать, что при включении параметров `Windows Restricted Chars` (располагается в настройках приложения **Carrot Server**) и `Event Autonaming`  невозможно использовать настройку **"Use for Autonaming"** в шаблоне, содержащем переменные типа **RichText** и **Color**. В случае включения настройки **"Use for Autonaming"** наблюдается некорректная работа формируемых ивентов шаблона.
...
>Параметр `Show preview player` осуществляет функционирование совместно с приложением **Carrot Preview Engine**.
>
>В случае использования функционала параметра `Show preview player` на удалённой рабочей станции, необходимо произвести настройку параметров `Server Host List` (указать IP-адрес рабочей станции на которой запущен **Carrot Server**) и `Stream Host` (указать IP-адрес рабочей станции на которой запущен **Carrot Preview Engine**) приложения **Carrot Preview Engine**.
>
>Важно учитывать, что при использовании функциональных возможностей параметра `Show preview player`, его цветовое отображение может не соответствовать генерируемую видеосигналу в приложении **Carrot Engine**. 
