...
>***ВАЖНО!*** 
>
>Возможность записи изображения в Carrot Engine после включения параметра Allow Recording реализована с помощью вспомогательной кнопки в окне Tools программного модуля Launcher.

* Recording Directory - предназначен для указания директории (пути), в которой будет хранится записанное изображение;
* Recording FPS - предназначен для настройки записи изображения в Carrot Engine при определённом количестве кадров;
* Recording Bitrate - предназначен для указания битрейта (количества бит, используемых для передачи/обработки данных в единицу времени) при записи изображения в Carrot Engine. Измеряется в кбит/с;
* MaxBitrate - предназначен для указания максимального битрейта при записи изображения в Carrot Engine. Измеряется в кбит/с.

**1.1.8. Раздел Render**

Раздел **Render** содержит следующие настраиваемые параметры:

* Initial Render Target Width - предназначен для указания ширины *рендер таргета*;
...
* Initial Render Target Height - предназначен для указания ширины *рендер таргета*;
* Use Legacy Text - предназначено для включения старых версий текста, используемых в Carrot Engine;
* Propagation of Invisibility - предназначен для включения PreComp рендер сцен, которые невидимы на временной шкале.

**1.1.9. Раздел Resource Manager**

Раздел **Resource Manager** содержит следующие настраиваемые параметры:

* `Unloading Event Type` - предназначен для выбора типа смены статуса эвента в состояние "Unloaded" после его остановки;

>***ВАЖНО!*** 
>
>`Unloading Event Type` содержит несколько настроек:
>* `Manual` - 
>* `Immediatly`
>* `After Timer`
>* `EventsPlayedAfter`
