function getVersionPath(path, versions, defaultVersion) {
  ...
  const version = versions.find((v) => v.folder === parts[2]); // part 1 is "carrot-documentation", part 2 is version folder name
...
function versionedDocsPlugin(hook, vm) {
  ...
  hook.doneEach(function () {
    ...
    if (version) {
      ...
      if (cover) {
        cover.innerHTML = cover.innerHTML.replace(
          /{{versionLabel}}/g,
          versionLabel
        );
      }
...
(function () {
  if (
    ...
    window.location.pathname[1] === "/index.html"
  ) {
