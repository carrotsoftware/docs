...
В данном случае, пользователю предлагается рассмотреть пример с участием 3-ёх вычислительных машин (серверов) со следующими IP-адресами:

* `192.168.1.1` - будет являться *"последним"* резервным серверном;
* `192.168.1.2` - будет являться *"главным" (мастером)* сервером;
* `192.168.1.3` - будет являться *"первым"* резервным сервером.
...
* в параметр `Replication Hosts` - `192.168.1.2, 192.168.1.3, 192.168.1.1`;
...
В раздел `Replication` для вычислительной машины с IP-адресом `192.168.1.2` ввести:

* в параметр `Replication Hosts` - `192.168.1.2, 192.168.1.3, 192.168.1.1`;
* в параметр `Local IP for Replication` - `192.168.1.3`.
...
В результате введённых настроек для каждой из вычислительных машин будет достигнут следующий функционал взаимодействия между ними при репликации (синхронизации) данных:

* при внезапном прекращении работы мастер-сервера (`192.168.1.2`), происходит переключение на первый резервный сервер (`192.168.1.3`);
* при внезапном прекращении работы первого резервного сервера (`192.168.1.3`), происходит переключение на второй (последний) резервный сервер (`192.168.1.1`).
